...
function ReasoningPanel({ text, live }: { text: string; live: boolean }) {
  ...
  // CONCLUÍDO: chip → abre o modal NATIVO do Obsidian (bottom sheet no mobile,
  // posição/animação de fábrica — o sheet custom portalado abria no topo).
  return (
    <button
      type="button"
      className="axxa-reasoning-chip"
      onClick={() => new ReasoningModal(app, text).open()}
