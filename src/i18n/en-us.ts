...
export const EN_US = {
  ...
  settings: {
    ...
    balanceSetAnchor: "add a top-up →",
    balanceEstimate: "estimated",
    balanceReal: "real",
    balanceLiveHint: "real balance straight from the API (click Refresh)",
    balanceCreditAmount: "$ loaded",
    balanceAddCredit: "+ Top-up",
    balanceRemoveCredit: "Remove top-up",
    balanceNote:
      "Log each top-up you load (date + amount). Balance = sum of top-ups − spend since the oldest top-up. “real” uses the provider's billing (OpenAI/Anthropic admin · OpenRouter native); otherwise it's “estimated” from vault chats. Note: the estimate only counts chats present in the vault since that date; the real figure depends on the provider's costs-API window.",
