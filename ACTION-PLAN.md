...
> **Status:** 🟡 Em andamento — Módulos 0–4 ✅, 6.1+6.2 ✅ Agent 6-provider, 6.4 ✅ RAG multimodal, Sprints J/K.1/K.2/K.3 ✅, **K.4 ✅** (image gen + Usage tab), **L ✅** (deep polish mobile + fullscreen saga), **M ✅** (Effort Engine + fullscreen v2) e **RAG v2 ✅** (v0.1.78–82: vault_search no agent · Obsidian-native · quantização selecionável · hybrid + grafo · auto-reindex) · próximo: **Sprint N — Validação & Estabilização → Coder Mode (6.3) → caminho do Alpha** (ver seção 🎯 PRÓXIMOS PASSOS)  
> **Versão:** 1.6 · plugin em **v0.1.82 (6 providers · 3 modos com streaming · multimodal · geração de imagem funcional · aba Usage com custos em USD · Effort Engine configurável por nível · fullscreen mobile + chrome polish do OS · wikilink/áudio entram como anexo · erro não polui contexto · Coder/Study mock · agent busca o RAG · índice Obsidian-native · quantização selecionável · busca híbrida · reindex automático · RAG v2 ✅ · chat: copiar/buscar/continuar/persona/editar/ramificar)**  
> **Última revisão:** 09/06/2026 (pós-v0.1.82 — RAG v2 fechado)  
...
| 09/06/2026 | v0.1.89 — RAG: + modelos de embed (Gemini · NIM · ada-002) | `EmbeddingProvider` += gemini/nim. `embedOpenAICompat` genérico (OpenAI/Gemini/NIM compartilham o shape `/embeddings`) — Gemini via `/v1beta/openai/`, NIM via `integrate.api.nvidia.com` com `input_type:passage`. Novos modelos: gemini-embedding-001 (3072, dim configurável), text-embedding-004 (768, free), nv-embedqa-e5-v5 (1024), llama-3.2-nv-embedqa (2048), ada-002 (legacy). Creds gemini/nim propagadas a indexação/busca/auto-reindex. **Fecha o backlog "embeddings Gemini/NIM".** |
| 09/06/2026 | v0.1.88 — Fix: busca vira modal (não abria) | O campo inline de busca era frágil (só aparecia com chat aberto, discreto). Refeito como **SuggestModal nativo** (`ChatSearchModal`) — abre sempre, filtra user/ai-response, e ao escolher **PULA + DESTACA** a mensagem na ChatArea (`data-msg-id` + `scrollIntoView` + flash 2.2s). |
| 09/06/2026 | v0.1.87 — Chat: editar mensagem + ramificar respostas | **Editar:** menu "Editar" ou double-click numa user-msg → textarea inline → "Salvar e reenviar" trunca a conversa dali e regenera. **Ramificar:** regenerar guarda a versão anterior como variante e gera a nova na MESMA bolha; navegação ‹ N/M › no footer (store: `beginVariant`/`syncVariant`/`navigateVariant`). Trade-off: o regen monta history textual (não re-faz vault search no vault-qa). Fecha a frente **"Editar & ramificar"**. |
| 09/06/2026 | v0.1.86 — Chat: persona por conversa | System prompt custom por chat (`sessionPersona` no store + `persona:` no frontmatter do `.md`). `PersonaModal` (textarea, Ctrl+Enter salva) acessível pelo menu "..." → substitui o prompt base no Chat/Vault Q&A e prefixa no Agent. Persiste e restaura ao recarregar. Frente "Persona por chat". |
| 09/06/2026 | v0.1.85 — Fix: hold-to-record acidental no scroll | O mic disparava ao descer o scroll perto do botão (o hold começava no `touchstart` imediato). Agora exige **hold deliberado de 180ms PARADO**; movimento >10px (scroll) ou tap rápido cancela antes de gravar. Guard via `micHoldTimerRef` + `micStartPosRef` (arm/move/end) no Composer. |
| 09/06/2026 | v0.1.84 — Chat polish: continuar resposta cortada (frente ✅) | Heurística detecta truncamento (output ≈ maxTokens·0.95) e marca a ai-response com `truncated` (+ `setTruncated` no store). Botão **"Continuar"** no AIResponse emenda a continuação **na MESMA bolha** (history até a resposta + nudge "continue de onde parou" → `appendToMessage`). Re-avalia truncamento (cortou de novo → botão fica). **Frente "Polish de UX" do Chat fechada:** copiar conversa (83) + busca (83) + continuar (84). |
| 09/06/2026 | v0.1.83 — Chat polish: copiar conversa + busca | Menu "..." do Header ganha **"Copiar conversa"** (markdown `## Você`/`## Assistente` → clipboard). Novo **ícone de busca** no Header → campo acima da ChatArea filtra user/ai-response que contêm o termo, com contador de resultados. Frente "Polish de UX" do Chat (escolha do dev). |
| 09/06/2026 | v0.1.82 — RAG v2 (5/5 ✅): reindex automático (opt-in) | Listeners `vault.on(modify/create/delete/rename)` → debounce 4s → `indexVault` incremental (re-embeda só hash-diferente, remove deletados). **Opt-in** (`settings.ragAutoReindex`, default off) — re-embed custa tokens e só roda com índice já criado. Toggle em Settings → RAG vale na hora (listeners sempre registrados, `schedule` checa o setting). Cancela reindex anterior via `AbortController`. **🎯 RAG v2 FECHADO:** vault_search no agent (v0.1.78) · índice Obsidian-native (79) · quantização selecionável int8/dim estilo Effort (80) · chunking estrutural + hybrid RRF + grafo (81) · reindex automático (82). |
| 09/06/2026 | v0.1.81 — RAG v2 (4/n): chunking estrutural + hybrid + grafo | **Chunking estrutural** (`chunkMarkdown`): quebra por heading (#..######) e leva o breadcrumb ("Arquitetura > RAG") no texto embedado. **Hybrid search** (`src/rag/hybrid.ts`): funde semantic (cosine) + keyword via Reciprocal Rank Fusion (k=60) — pega conceito/sinônimos E termos raros/nomes próprios. **Graph-aware ranking:** resultado linkado por outro resultado relevante ganha boost (co-citação, via `metadataCache` em memória, sem I/O). Vault Q&A e `vault_search` agora SEMPRE fundem (não só fallback). |
| 09/06/2026 | v0.1.80 — RAG v2 (3/n): quantização selecionável (estilo Effort) | Vetores viram **typed arrays** unit-normalizados — `Int8Array` (8× menos RAM que o `number[]` float64 antigo) ou `Float32Array`, conforme o **perfil**. 4 perfis (🎯 Precisão · ⚖️ Equilibrado · 🪶 Leve · 🔬 Mínimo) combinam precisão + dim (Matryoshka via param `dimensions` da OpenAI). Settings → RAG mostra o perfil **recomendado pelo nº de notas** (⭐) + "melhor uso", igual o Effort. Persistência base64 binária (`INDEX_VERSION` 2 → reindex). Score: cosine = dot (int8 ÷ 127²). Modelos sem Matryoshka (Nemotron/Ollama) caem p/ int8 dim-cheia, com aviso. Núcleo em `src/rag/quant.ts`. **25k chunks: ~307 MB → ~13 MB** (Leve int8/512d). **Falta no RAG v2:** reindex automático por evento, chunking estrutural, hybrid search, context expansion via grafo de links. |
